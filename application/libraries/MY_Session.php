...
//
//  A PHP Driver based version of sessions for mongodb
//  Bennington Purcell - Sourcemap
//  Sourcemap.com
//  2012-10-25
//
...
class MY_Session extends CI_Session
{
    ...
    function sess_read()
    {
        ...
        if ($this->sess_use_mongo_database === TRUE):
            ...
            $m = new Mongo();
            $c = $m->local->{$this->sess_collection_name};
            $query = $c->findOne($where);
    ...
    function sess_write()
    {
        ...
        $m = new Mongo();
        $c = $m->local->{$this->sess_collection_name};
        $c->update(array('_id' => new MongoId($this->userdata['session_id'])), array('$set' => array('last_activity' => $this->userdata['last_activity'], 'user_data' => $custom_userdata)));
    ...
    function sess_update()
    {
        ...
        if ($this->sess_use_mongo_database === TRUE)
        {
            ...
            $m = new Mongo();
            $c = $m->local->{$this->sess_collection_name};
            $current = $c->findOne(array('_id' => new MongoId($old_sessid)));
            
            $current['_id'] = new MongoId($new_sessid);
            $m = new Mongo();
            $c = $m->local->{$this->sess_collection_name};
            $c->remove(array('_id' => new MongoId($old_sessid)));
    ...
    function sess_destroy()
    {
        ...
        if ($this->sess_use_mongo_database === TRUE AND isset($this->userdata['session_id'])):
            $m = new Mongo();
            $c = $m->local->{$this->sess_collection_name};
            $c->remove(array('_id' => new MongoId($this->userdata['session_id'])));
        endif;
    ...
    function _sess_gc()
    {
        ...
        if ((rand() % 100) < $this->gc_probability):
            ...
            $m = new Mongo();
            $c = $m->local->{$this->sess_collection_name};
            $c->remove(array('last_activity' => array('$lt' => $expire)));
